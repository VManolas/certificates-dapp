...
import { useEmployerRegistration, useEmployerInfo, useIsVatAvailable } from '@/hooks/useEmployerRegistry';
...
export default function EmployerRegister() {
  ...
  const [vatCheckValue, setVatCheckValue] = useState<string>('');
  ...
  const debouncedVatCheck = useCallback(
    debounce((vat: string) => {
      if (vat && vat.length >= EMPLOYER_VALIDATION.vatNumber.minLength) {
        setVatCheckValue(vat);
      } else {
        setVatCheckValue('');
      }
    }, 500),
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Trigger debounced VAT check
    if (name === 'vatNumber') {
      debouncedVatCheck(value);
    }
  };
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                />
                {/* VAT Availability Indicator */}
                {vatCheckValue && vatCheckValue === formData.vatNumber && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    {isCheckingVat ? (
                      <svg className="animate-spin h-5 w-5 text-surface-400" viewBox="0 0 24 24">
